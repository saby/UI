...
/**
 * Подписываемся на изменения шаблона и полей на инстансе
 * @param {Control<P, S>} inst
 * @param {ITemplateFunction} template
 */
export function reactiveObserve<P = IControlOptions, S = {}>(inst: Control<P, S>, template: ITemplateFunction): void {
        ...
        templateProps.forEach((prop) => {
            ...
            Object.defineProperty(inst, prop, {
                ...
                set: function reactiveSetter(value: unknown): void {
                    if (inst.reactiveValues[prop] !== value) {
...
/**
 * Следим за изменениями в версионируемых объектах, и обно
 * @param {Control} inst
 * @param {string} prop
 */
 ...
 * на свойстве добавляется поле _$reactived которое хранит ссылку на инстанс
 ...
 * @param {Control} inst
 */
export function releaseProperties(inst: Control): void {
